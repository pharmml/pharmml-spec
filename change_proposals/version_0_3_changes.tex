...
PharmML-NMTRAN translation team and many other communications.
...
\begin{itemize}
\item
Part I -- Mapping of the columns defined in a dataset
\item
Part II -- Referencing the NONMEM-format dataset (as an external file or inline data)
\item
Part III -- Code injection (with symbol mapping)
\end{itemize}

\subsection{Part I -- Mapping of the columns in a dataset}
\label{subset:partI}
...
\xatt{ oid="NMoid"} assigned to the \xelem{NONMEMdataSe} element, it's use will be explained in the section 
\ref{sec:modellingSteps}.
\lstset{language=XML}
\begin{lstlisting}
    <!-- MODELLING STEPS -->
...
\subsection{Part II -- Referencing the NONMEM-format dataset}
...
The following is new and specific to Part III. The first section of the next listing shows 
...
\section{Changes in structure of \xelem{ModellingSteps}}
\label{sec:modellingSteps}
You might have noticed that the \xelem{NONMEMdataSet} in sections \ref{subset:partI} and 
\ref{subset:partIII} is placed directly under the \xelem{ModellingSteps}. The reorganisation 
of the code is suppose to provide more flexibility to the encode tasks and workflows, in that
first multiple data sources and tools can be defined which are referred to later in the estimation 
or simulations steps, \xelem{EstimationStep} and \xelem{SimulationStep}, respectively. 
This section illustrates how this can work. 

Let's consider that one would like to encode a simple workflow consisting of estimation and simulation 
tasks. The first based on the NONMEM format dataset, the second using user defined R-script. 
...
\begin{lstlisting}
    ...
    <ModellingSteps xmlns="http://www.pharmml.org/2013/03/ModellingSteps">
        ...
        <NONMEMdataSet oid="NMoid">
            <ColumnMapping>
                <!-- ommited mapping the data set declaration -->
        </NONMEMdataSet>
        ...
        <EstimationStep oid="estStep">
            ...
            <ParametersToEstimate>
                <!-- ommited details on parameter estimations -->
            </ParametersToEstimate>
...
\begin{description}
\item[Note] There was a discussion on how to improve this section during of the PharmML sessions 
at the last consortium meeting, 25-28 March 2014. Unfortunately, due to time constraints the changes
...
\begin{figure}[ht!]
\centering
 \includegraphics[width=120mm]{pics/IOV_2levels_treeOnly}
...
For example, \xelem{NONMEMdataSet} described in section \ref{subset:partI} is annotable down to 
