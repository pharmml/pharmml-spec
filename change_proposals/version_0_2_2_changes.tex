...
Additionally to values in Table \ref{tab:MDLPharmML_columnTypes} and \ref{tab:PharmML_columnTypes}, the \xatt{undefined} value can be used, this is most likely to be utilised when defining external tools datasets.
       
\subsection{Part III -- NMTRAN code injection (Chan et al.)}
To understand the purpose of this module, it is helpful to consider the following \cite{Chan:2005fk} example first.

%"Exogenous oral levodopa was modeled by a steady state infusion with constant but unknown rate, which ended at the start of the first levodopa infusion of each trial. The rate of steady state infusion (R1) was calculated from: R1 = CL Ѕ Css\_pre"
%
%This is encoded in the data file using flag '-1' (starting with month 6) in the RATE column, e.g.
%
%\lstset{language=NONMEMdataSet}
%\begin{lstlisting}
%\#ID	MONTH	TRIAL	OCC	WTKG	PREV	TIME	AMT	RATE	SS	II	CMT	DV	MDV
%552	6	2	3	70	2	409	0	-1	1	0	1	.	1
%\end{lstlisting}
%
%and the unknown rate, specified in the 'R1' in the NMTRAN code:
%
%\lstset{language=NONMEMdataSet}
%\begin{lstlisting}
%  IF (PREV.EQ.0) THEN
...
The first two parts of the code, i.e. mapping of dataset columns and referencing of the NONMEM file, are analog to the previous example. Only symbols appearing in the \xelem{ModelDefinition} 
will be mapped using the generic \xelem{ColumnMapping} element. All other symbols, appearing in the dataset but not in the model, have an appropriate attribute \xatt{columnType} which identifies their type.
...
\subsection{Minimal form of external tools support}
Following items can be implemented to provide the minimum information needed for an external target tool support
\begin{itemize}
\item
object identifier -- can used to reference the tool from a task in the \xelem{ModellingSteps}
\item
target tool name
\item
URL
...
\subsection{New attribute \xatt{columnType} in the \xelem{DataSet}/\xelem{Definition}}
...
\end{align}
\end{itemize}
This new input type has the attribute \xatt{variable}. It was requested to rename the \xatt{inputType} attribute and it's values -- the following table summarises the changes
...
The new type \xatt{variable} can be used in the new \xelem{LookupTable} block, see also example in section \ref{subsec:caseC1}.
