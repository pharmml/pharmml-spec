...
target tool name
\item
target tool data import with file name and URL only
\end{itemize}
...
\begin{lstlisting}
        ...
        <TargetTool oid="someOID">
            <TargetToolName>R</TargetToolName>
            
            <ds:TargetToolData>
                <ds:ImportTargetData oid="itt">
                    <ds:name>someName</ds:name>
                    <ds:url>someURL</ds:url>
                </ds:ImportTargetData>
            </ds:TargetToolData>
...
chosen from a list: \emph{\{constant, linear, spline, pchip, cubic\}}
...
is given as a lookup table defined in the \xelem{TrialDesign}
\begin{table}[H]
\begin{center}
\scriptsize
\begin{tabular}{lllll}
...
a. \xatt{type} attribute values in the \xelem{VariabilityReference}
- \xatt{model} $\longrightarrow$ \xatt{parameterVariability}
- \xatt{error} $\longrightarrow$ \xatt{residualError}

b. \xatt{inputType} attribute in \xelem{DoseAmount}
\xatt{inputType} $\longrightarrow$ \xatt{inputTarget}
and its values
- \xatt{target} $\longrightarrow$ \xatt{derivativeVariable}
- \xatt{dose} $\longrightarrow$ \xatt{parameter}
plus new type introduced (can also be used in the new \xelem{LookupTable}):
- \xatt{variable}
