...
Beside the values in Table \ref{tab:MDLPharmML_columnTypes} and \ref{tab:PharmML_columnTypes}, the \xatt{undefined} value can be used, this is most likely to be utilised when defining external tools datasets for which support is very limited and their needs underspecified.
...
Two levodopa administrations were modelled, one of them as so called \emph{Exogenous oral levodopa administration}. 
"Exogenous oral levodopa was modeled by a steady state infusion with constant but unknown rate, 
which ended at the start of the first levodopa infusion of each trial. The rate of steady state infusion (R1) 
was calculated from: R1 = CL Ѕ Css\_pre". 
...
Following items can be implemented to provide the minimum information needed for an external target tool support
\begin{itemize}
\item
...
\url{http://www.mathworks.co.uk/help/matlab/ref/interp1.html}. Additionally the \emph{constant} algorithm type is supported.}. 
In case \emph{C2} the interpolation 
...
\end{tabular}
\end{center}
\caption{The table summarises possible configurations of what is given as input, column \emph{Setup/Input} and how it can be structured and implemented, columns \xelem{TrialDesign} and \emph{Target/ModelDefinition}.}
\label{tab:C1C2C2}
\end{table}
...
The last part, the \xelem{LookupTable} definition and mapping, is identical to the previous example and will be skipped here.
...
PPV_CL=0.1,
0.01, PPV_V=0.1,
0, 0.01, PPV_EMAX=0.01,
0, 0, 0.01, PPV_C50=0.01
} # end matrix struc1
\end{lstlisting}
...
\begin{align}
\begin{bmatrix} x & y & y \\ z & v & y \\ z & v & y \end{bmatrix} \nonumber
\end{align}
...
\begin{lstlisting}
            <Correlation>
                ...
                <Matrix matrixType="CovarianceMatrix">
                    <ct:RowNames>
...
A possibility of specifying one of five standard interpolation algorithms is provided, {constant, linear, spline, pchip, cubic}, as defined in MATLAB, http://www.mathworks.co.uk/help/matlab/ref/interp1.html

\subsection{The notation of initial conditions for ODE's is extended}
Last versions of PharmML assumed the initial time, $t_0$, to be always equal $0$. This restriction is now relaxed and PharmML allows to store any initial time. The initial value and time get their proper tags, \xelem{InitialValue} and \xelem{InitialTime}, accordingly. In both cases, the user can define them as numerical values, symbols defined somewhere else in the model or arbitrary mathematical expressions.
...
IC 	&	$A_0$				&	$A(t_0=a) = A_0$ \\
  ...
  \hline
\end{tabular}
\caption{Summary of \emph{initial conditions} in ODE support in PharmML. Version 0.2.1 supported the implementation of initial values only, version 0.3 supports additionally the storage of initial times $t_0$.}
\end{center}
\end{table}
